...
<style>
  ...
  *{box-sizing:border-box}
  ...
  #panel{top:20px;left:20px;width:var(--panel-w,328px);
         max-height:calc(100vh - 40px);display:flex;flex-direction:column;
         overflow:hidden}
  #panel-head{display:flex;align-items:flex-start;gap:10px;flex:none;
              padding:14px 13px 9px 17px}
  #panel h1{margin:0;flex:1;font-size:17px;font-weight:600;letter-spacing:-.2px}
  #panel p{margin:0;font-size:12.5px;color:var(--ink-2)}
  /* the body rolls up like a blind: on a phone the panel owns half the screen,
     and rolling it away gives that half back to the map without losing the
     controls entirely */
  #panel-body{flex:1 1 auto;min-height:0;overflow-y:auto;padding:0 17px 15px;
              max-height:100vh;overscroll-behavior:contain;
              transition:max-height .28s ease,opacity .18s ease}
  #panel.rolled #panel-body{max-height:0;opacity:0;overflow:hidden;
                            padding-top:0;padding-bottom:0}
  #roll{flex:none;width:26px;height:26px;padding:0;display:grid;
        place-items:center;background:none;border:1px solid transparent;
        border-radius:6px;color:var(--ink-2);cursor:pointer}
  #roll:hover{background:var(--panel-2);border-color:var(--line);color:var(--ink)}
  #roll svg{width:15px;height:15px;fill:none;stroke:currentColor;stroke-width:2;
            stroke-linecap:round;stroke-linejoin:round;
            transition:transform .28s ease}
  #panel.rolled #roll svg{transform:rotate(180deg)}
  /* drag the right edge to narrow the panel, the way a window resizes */
  #grip{position:absolute;top:0;right:0;width:9px;height:100%;cursor:ew-resize}
  #grip::after{content:'';position:absolute;top:10px;bottom:10px;left:3px;
               width:3px;border-radius:2px;background:transparent;
               transition:background .15s}
  #grip:hover::after,#grip.dragging::after{background:var(--line-2)}
...
<div id="panel" class="card">
  <div id="panel-head">
    ...
    <button id="roll" type="button" aria-expanded="true" aria-controls="panel-body"
            title="Roll the panel up" aria-label="Roll the panel up">
...
// bearing turns the camera around this point, so it has to be the middle of
// the city or Milan slides across the screen while it spins instead of turning
// on the spot. Chosen by minimising how far the outline's centre moves over a
// full turn: 434px of travel from the point that merely framed it well, 110px
// from here — and the rest is the outline itself, which is not round.
var CITY_LON = 9.165;
var CITY_LAT = 45.47;
...
rollButton.addEventListener('click', function () {
  var rolled = panel.classList.toggle('rolled');
  var label = rolled ? 'Roll the panel down' : 'Roll the panel up';

  rollButton.setAttribute('aria-expanded', String(!rolled));
  rollButton.setAttribute('aria-label', label);
  rollButton.title = label;
});
...
grip.addEventListener('pointerdown', function (event) {
  ...
  // never wider than half the window: the panel is an aside, not the page
  var maxWidth = Math.max(MIN_PANEL, window.innerWidth / 2);

  function drag(moved) {
    var width = startWidth + (moved.clientX - startX);

    panel.style.setProperty(
      '--panel-w',
      Math.round(Math.min(maxWidth, Math.max(MIN_PANEL, width))) + 'px'
    );
  }
